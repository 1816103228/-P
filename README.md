...
- **边说边答**：浏览器用 Web Speech API 持续识别（zh-CN），每识别出一句话立刻发给语音服务；DeepSeek 流式回复逐字推回，服务端把句子**合并成 2-3 句一段**（连接数少、句间语气连贯）并用 edge-tts 边合成边推送，小音频块**聚合成约 2 秒的大单元**再下发，浏览器拿到第一块就开始播报（零间隙衔接），既快又稳，不等整段生成完。
...
- **开口即打断**：播报期间识别会暂停（防止回声自激），改用**带回声抑制的麦克风音量检测（VAD）**——持续跟踪本段播报的回声/噪声基准、校准后需先安静片刻再武装、连续 240ms 超阈值才判定，大幅降低"莫名被打断"；你正常说话即可打断并取消未完成的生成，点按钮同样可以打断。
...
- **环境要求**：语音识别依赖浏览器 Web Speech API，请使用 Chrome 或 Edge，并直接访问 `127.0.0.1:8765`（本机地址属于安全上下文，麦克风权限正常）；需先启动 `voice_server`（`scripts/start.bat` 会自动启动）。
